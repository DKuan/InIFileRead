...
// 创建日期:		2017/01/05
// 创 建 人:		
// 函数说明:		get the array data
// 返 回 值:   	void
//************************************************************************
void CMyINI::Get_Array(string root, string key, float *Data, int num)
{
	int pos_l = 0;
	int pos_r = 1;
	int length = 4;
	...
	map<string, string>::iterator sub_itr = itr->second.sub_node.find(key);
	if (!(sub_itr->second).empty())
		Data_s = sub_itr->second;
		for (int i = 0; i < num; i++)
		{
			pos_r = Data_s.find(',', pos_l);
			Data_sub = Data_s.substr(pos_r - length, length);
			Data[i] = stof(Data_sub);
			pos_l = pos_r + 1;
		}
}
